...
App.DescriptionMaterialSimpleView = Marionette.LayoutView.extend({
    ...
    updateMlBtnDisplay:function(){
        ...
        if(this.$el.hasClass('portfolioMedia')){
            if(this.materials && this.materials.size() > 6){
                this.$el.find(".less-more-button").show();
            }else{
                this.$el.find(".less-more-button").hide();
            }
        }else{
            if(this.materials && this.materials.size() > 4){
                this.$el.find(".less-more-button").show();
            }else{
                this.$el.find(".less-more-button").hide();
            }
...
//------------------Activity Models-----------------------------------------
App.activityTags = ["Admission links", "Transfer", "International students", "Life after / Graduate development", "Visit campus", "Contact ", "Research opportunity", "Admission stats", "Dorms and housing", "City info", "Faqs", "All"];
