...
public class AdmissionProfileRepositoryHibernateImpl extends HibernateRepository implements AdmissionProfileRepository {
    ...
    public WebPage<Object> queryMySchools(Integer userId, WebPage<Object> webPage) {
        ...
        try {
            ...
            String hql = "from UserFollow where fromId =:userId ";
            Query query = getSession().createQuery(hql);
            query.setParameter("userId", userId);
            List<UserFollow> userList = query.list();

            String sqlstr = "(";
            for(int i = 0; i < userList.size();i++){
                User user=  userList.get(i).getOwner();
                if(i == userList.size() -1 ){
                    sqlstr += "'" + user.getEmail() + "'" + ")";
                }else{
                    sqlstr += "'" + user.getEmail() + "'" + ",";
                }
            ...
            // 查询到的记录总数
            Integer totalRow = Long.valueOf(userList.size()).intValue();
            webPage.setTotalCount(totalRow);
            webPage.setPageSize(5);
            double c = Math.ceil((float) totalRow / (float) webPage.getPageSize());
            webPage.setTotalPage((int) c);
            ...
            Query query1 = getSession().createQuery("FROM AdmissionProfile where email in "+sqlstr+" order by name");
            query1.setFirstResult((webPage.getCurrentPage() - 1) * webPage.getPageSize());
            query1.setMaxResults(webPage.getPageSize());
